...
namespace Crmleaf\Payroll\Tools\RoiCalculator\Tests;

use Crmleaf\Payroll\Calculators\RoiCalculator;
use Crmleaf\Payroll\Tools\RoiCalculator\RoiCalculatorServiceProvider;
 ...
 */
final class RoiCalculatorTest extends TestCase
{
    /**
     * Held as a literal rather than as RoiCalculator::class so that a calculator
     * which does not exist fails as one readable assertion instead of a fatal
     * error that takes the rest of the file with it.
     */
    private const CALCULATOR = 'Crmleaf\Payroll\Calculators\RoiCalculator';
    ...
    {
        return [RoiCalculatorServiceProvider::class];
    }
    ...
    protected function defineEnvironment($app): void
    {
        ...
        // pins down.
        $app['config']->set('roi-calculator.route.enabled', true);
    ...
    {
        $calculator = $this->app->make(RoiCalculator::class);

        self::assertInstanceOf(RoiCalculator::class, $calculator);
        self::assertSame($calculator, $this->app->make(RoiCalculator::class), 'The binding should be a singleton.');
    }

    public function test_the_configuration_is_merged(): void
    {
        self::assertSame('roi-calculator', $this->app['config']->get('roi-calculator.route.name'));
        self::assertSame('Payroll ROI Calculator', $this->app['config']->get('roi-calculator.view.title'));
    }

    public function test_the_route_is_off_until_it_is_asked_for(): void
    {
        /** @var array{route: array{enabled: bool}} $shipped */
        $shipped = require __DIR__.'/../config/roi-calculator.php';
    ...
    {
        $html = Blade::render('<x-crmleaf::roi-calculator />');

        self::assertStringContainsString('data-crmleaf-tool="roi-calculator"', $html);
    ...
    {
        $response = $this->postJson('/tools/roi-calculator', [
            'employee_count' => 250,
            'hourly_rate' => 900,
        ...
        $response->assertJsonPath('tool', 'roi-calculator');
    }

    public function test_incomplete_input_is_rejected_rather_than_guessed_at(): void
    {
        $this->postJson('/tools/roi-calculator', [])
            ->assertStatus(422)
