# Contributing to Payroll ROI Calculator
...
```bash
git clone https://github.com/crmleaf/roi-calculator.git
cd roi-calculator
composer install
composer test
```
...
requires. `Crmleaf\Payroll\Calculators\RoiCalculator` is the class doing the work; everything else
...
Made in the monorepo, against `tools/roi-calculator/`:
...
```
fix(roi): <what changed>
test(roi): <the case now covered>
```
