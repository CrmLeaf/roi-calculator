...
<head>
    ...
    <title>{{ $title ?? 'Payroll ROI Calculator' }}</title>
    <meta name="description" content="Quantify the return on moving off manual payroll.">
    <style>
...
<x-crmleaf::roi-calculator
    ...
    :result="$result ?? null"
    :error="$error ?? null"
    :heading="$title ?? 'Payroll ROI Calculator'"
/>
...
<footer class="crmleaf-tool__colophon">
    ...
    <p>
        Computed by <a href="https://github.com/crmleaf/roi-calculator">crmleaf/roi-calculator</a>,
...
{{--
    ...
    The bundled script binds any element carrying data-crmleaf-tool="roi-calculator",
    computes on submit, and only falls back to posting the form if the browser
    build is not present.
--}}
@if (config('roi-calculator.assets.script', true))
    @if (config('roi-calculator.assets.cdn'))
        <script src="{{ config('roi-calculator.assets.cdn') }}" defer></script>
    @endif
    <script src="{{ asset('vendor/roi-calculator/roi-calculator.js') }}" defer></script>
@endif
